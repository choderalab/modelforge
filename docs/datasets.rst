...
- `element_filter`: A filter to select systems with or without certain elements, which are denoted by atomic numbers. Negative values indicates atomic numbers to be excluded. e.g. [[29, -78], [20, 17]] selects (a) systems containing copper but not platinum, and (b) systems containing zinc and chloride. Leave this field as an empty list or remove it will disable this element filtering feature.