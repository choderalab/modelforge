...
class TorchDataset(torch.utils.data.Dataset[BatchData]):
    ...
    def _load_properties(
        ...
        print("herhehreheherheheh")
        print(dataset[property_name.total_charge])
        ...
        properties["quadrupole_moment"] = (
            torch.from_numpy(dataset[property_name.quadrupole_moment])
            if property_name.quadrupole_moment is not None
            else torch.zeros(
...
class DataModule(pl.LightningDataModule):
    ...
    def _per_datapoint_operations(
        ...
        # note, if we are also shifting the energies, we can calculate the min/max/mean
        # in the same loop to avoid multiple passes over the dataset
